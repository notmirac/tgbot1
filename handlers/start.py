...
async def btn_subscription_status(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        await message.answer(
            ...
            else "⚠️ Error. Try again.\n\nIf buttons broke, send /start or /sstart.",
            reply_markup=main_menu_keyboard(lang),
        )
...
# ══════════════════════════════════════════════════════════════
#  ПОДДЕРЖКА
#  Исправлено: сообщение пользователя приходит к админу
#  с кнопкой «Ответить». Данные пользователя видны только
#  администратору. Пользователь данных админа не видит.
# ══════════════════════════════════════════════════════════════
...
async def support_send(message: Message, state: FSMContext) -> None:
    ...
    # Импортируем reply_kb из admin чтобы не создавать циклический импорт
    from handlers.admin import reply_kb

    try:
        # Отправляем админу с кнопкой «Ответить»
        # В сообщении — данные пользователя (только для админа)
