...
from database import add_or_update_user, has_active_subscription, get_subscription_expires, has_profile, is_banned, get_user_lang, set_user_lang
...
async def cmd_start(message: Message, state: FSMContext) -> None:
    ...
    await add_or_update_user(user_id=user_id, username=message.from_user.username, first_name=message.from_user.first_name, language_code=(message.from_user.language_code or "ru"))
...
async def btn_subscription_status(message: Message, state: FSMContext) -> None:
    ...
    try:
        if await has_active_subscription(user_id):
            ...
            await message.answer(tr(lang, "sub_active", date=expires_str), reply_markup=main_menu_keyboard(lang), parse_mode="HTML")
        else:
            await message.answer(tr(lang, "sub_none"), reply_markup=buy_access_keyboard(_price_label(lang), lang), parse_mode="HTML")
    except Exception as e:
        logger.error("btn_status error: %s", e)
        await message.answer("⚠️ Ошибка. Попробуй ещё раз." if lang == "ru" else "⚠️ Error. Try again.", reply_markup=main_menu_keyboard(lang))
...
async def support_send(message: Message, state: FSMContext) -> None:
    ...
    try:
        await message.bot.send_message(
            config.admin_id,
            tr(lang, "support_admin_message", name=message.from_user.full_name, user_id=message.from_user.id, username=message.from_user.username or config.admin_username, lang=lang, text=text),
            parse_mode="HTML",
        )
    except Exception as e:
