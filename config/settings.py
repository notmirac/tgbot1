...
class Config:
    ...
    subscription_price: int = field(default_factory=lambda: int(os.getenv("SUBSCRIPTION_PRICE", "29900")))
    ...
    photo_18_ids: list[str] = field(default_factory=lambda: [p.strip() for p in os.getenv("PHOTO_18_IDS", "").split(",") if p.strip()])
