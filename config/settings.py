...
def _require(key: str) -> str:
    value = os.getenv(key)
    if not value:
        raise EnvironmentError(f"Переменная окружения '{key}' не задана. Проверь .env или переменные окружения.")
    return value
...
class Config:
    ...
    photo_18_ids: list[str] = field(default_factory=lambda: [x.strip() for x in os.getenv("PHOTO_18_IDS", "").split(",") if x.strip()])
    reply_delay_min: float = field(default_factory=lambda: float(os.getenv("REPLY_DELAY_MIN", "2.0")))
    reply_delay_max: float = field(default_factory=lambda: float(os.getenv("REPLY_DELAY_MAX", "5.0")))
